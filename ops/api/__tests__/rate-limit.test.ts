...
describe('checkRateLimit', () => {
  ...
  it('should allow requests when under the limit', async () => {
    ...
    const result = await checkRateLimit(mockEnv, testIP, testKey, maxRequests, windowSeconds);
  ...
  it('should allow exactly maxRequests within the window', async () => {
    ...
    const finalCheck = await checkRateLimit(mockEnv, testIP, testKey, maxRequests, windowSeconds);
  ...
  it('should block the (maxRequests + 1)th request', async () => {
    ...
    const blocked = await checkRateLimit(mockEnv, testIP, testKey, maxRequests, windowSeconds);
  ...
  it('should correctly filter requests outside the window', async () => {
    ...
    let result = await checkRateLimit(mockEnv, testIP, testKey, maxRequests, windowSeconds);
    ...
    result = await checkRateLimit(mockEnv, testIP, testKey, maxRequests, windowSeconds);
    expect(result.allowed).toBe(true);
  ...
  it('should return 0 remaining when denied', async () => {
    ...
    const denied = await checkRateLimit(mockEnv, testIP, testKey, maxRequests, windowSeconds);
  ...
  it('should handle concurrent requests correctly', async () => {
    ...
    const results = [];
    for (let i = 0; i < 10; i++) {
      results.push(await checkRateLimit(mockEnv, testIP, testKey, maxRequests, windowSeconds));
    }
  ...
  it('should isolate rate limits by IP and key', async () => {
    ...
    await checkRateLimit(mockEnv, '192.168.1.1', 'key1', maxRequests, windowSeconds);
    await checkRateLimit(mockEnv, '192.168.1.1', 'key1', maxRequests, windowSeconds);

    // Different IP should not be affected
    let result = await checkRateLimit(mockEnv, '192.168.1.2', 'key1', maxRequests, windowSeconds);
    expect(result.allowed).toBe(true);

    // Different key should not be affected
    result = await checkRateLimit(mockEnv, '192.168.1.1', 'key2', maxRequests, windowSeconds);
    expect(result.allowed).toBe(true);
  });
});
