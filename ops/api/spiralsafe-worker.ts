...
async function logRequest(
  ...
  try {
    ...
    await env.SPIRALSAFE_KV.put(logKey, JSON.stringify(logEntry), {
  ...
  async fetch(
    ...
    const corsHeaders = {
      ...
      "Access-Control-Allow-Headers":
        "Content-Type, Authorization, X-AWI-Intent, X-API-Key",
    };
...
function analyzeCoherence(
  ...
  return {
    ...
    regions,
    coherent:
      repetitionScore < t.curl_critical &&
      Math.abs(expansionScore) < t.div_critical,
  };
}
...
async function handleAWI(
  ...
  if (request.method === "POST" && path === "/api/awi/request") {
    ...
    const grant: AWIGrant = {
      ...
      expires_at: new Date(
        Date.now() + (body.ttl_seconds ?? 3600) * 1000,
      ).toISOString(),
      audit_trail: [
        {
          timestamp: new Date().toISOString(),
    ...
    await env.SPIRALSAFE_KV.put(`awi:${grant.id}`, JSON.stringify(grant), {
  ...
  if (request.method === "POST" && path === "/api/awi/verify") {
    ...
    const actionAllowed = grant.scope.actions.some(
      (a) =>
        a === "*" ||
        a === body.action ||
        body.action.startsWith(a.replace(/\*/g, "")),
    );
...
async function handleHealth(env: Env): Promise<Response> {
  ...
  checks.api_key_configured = !!(
    env.SPIRALSAFE_API_KEY && env.SPIRALSAFE_API_KEY.length > 0
  );
