...
- We include deterministic, offline mocks and unit tests to validate Grok-style function calls and web_search parsing:
  - `ops/integrations/xai-grok/mocks/grok_web_search_response.xml` (XML web_search response example)
  - `ops/integrations/xai-grok/mocks/grok_function_call_envelope.xml` (function-call envelope)
  - `ops/integrations/xai-grok/tests` contains `pytest` tests that assert parsing and measurable H&&S:WAVE improvements.
- Local test dependencies (recommended): `requests`, `pydantic`, `pytest` (see `ops/integrations/xai-grok/requirements.txt`).
- A CI workflow `/.github/workflows/test-grok.yml` runs these tests on the `integration/xai-grok` branch.
