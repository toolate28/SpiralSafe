...
async function executeLocal(command: string): Promise<{ stdout: string; stderr: string; exitCode: number }> {
  try {
...
async function executeRemote(command: string): Promise<{ stdout: string; stderr: string; exitCode: number }> {
...
async function handleMessage(sender: string, message: string): Promise<string> {
  ...
  if (parsed.type === 'local' || parsed.type === 'remote') {
    ...
    if (isDangerous(parsed.command)) {
      logger.warn({ sender, command: parsed.command }, 'Dangerous command blocked');
...
async function receiveMessages(): Promise<Array<{ sender: string; message: string }>> {
  try {
    const response = await axios.get(`${config.signal.apiUrl}/v1/receive/${config.signal.number}`);
